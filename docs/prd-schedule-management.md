...
A robust 4-step generator to build a single-elimination tournament bracket, with automatic bracket progression.
*   **Step 1 (Number of Teams)**: Select the number of teams and byes. Choose to auto-seed using standings or use manual placeholders (e.g. #1 Seed). Placeholders allow admins to build and save the bracket early in the season before final standings are known, then revisit later to assign real teams.
*   **Step 2 (Assign First-Round Matchups)**: Shows a visual bracket tree. Seeds are auto-assigned (1 vs 8, 2 vs 7, etc) with byes mapped to top seeds. Admins can adjust dropdowns.
*   **Step 3 (Enter Game Details)**: Assign Dates, Times, and Locations to all games in the bracket. Later round games show opponents as "Winner#2" etc.
...
*   **[NEW]** `PATCH /api/bash/admin/seasons/[id]/schedule/[gameId]` — Update a game. If it's a playoff game with a `nextGameId`, automatically update the downstream game's `homeTeam` or `awayTeam` (based on `nextGameSlot`) with the winning team's slug.
...
*   **[NEW]** `POST /api/bash/admin/seasons/[id]/schedule/playoffs` — Bulk insert for the Playoff Wizard. Creates linked games with `nextGameId`/`nextGameSlot` references. Overwrites any existing playoff games for the season (after confirmation).
